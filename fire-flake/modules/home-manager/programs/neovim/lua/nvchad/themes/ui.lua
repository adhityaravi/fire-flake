...
M.compact = function()
  ...
  -- Calculate visible range based on current index and limit
  local start_idx = math.max(1, state.index - math.floor(state.limit.compact / 2))
  local end_idx = math.min(#list, start_idx + state.limit.compact - 1)
  
  -- Adjust start if we're near the end
  if end_idx - start_idx + 1 < state.limit.compact then
    start_idx = math.max(1, end_idx - state.limit.compact + 1)
  end
...
M.flat = function()
  ...
  -- Calculate visible range based on current index and limit
  local start_idx = math.max(1, state.index - math.floor(state.limit.flat / 2))
  local end_idx = math.min(#list, start_idx + state.limit.flat - 1)
  
  -- Adjust start if we're near the end
  if end_idx - start_idx + 1 < state.limit.flat then
    start_idx = math.max(1, end_idx - state.limit.flat + 1)
  end
...
M.bordered = function()
  ...
  -- Calculate visible range based on current index and limit
  local start_idx = math.max(1, state.index - math.floor(state.limit.bordered / 2))
  local end_idx = math.min(#list, start_idx + state.limit.bordered - 1)
  
  -- Adjust start if we're near the end
  if end_idx - start_idx + 1 < state.limit.bordered then
    start_idx = math.max(1, end_idx - state.limit.bordered + 1)
  end
  
  local last_index = end_idx
